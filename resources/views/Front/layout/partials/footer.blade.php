...
<script>
    ...
    $(document).ready(function() {
                    ...
                    success: function(data) {
                        ...
                        if (data.length > 0) {
                            ...
                            $.each(data, function(index, product) {
                                searchHtml += '<li>'
                                searchHtml +=
                                    '<a href="#" class="flex items-center px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white list-group-item" data-id="' +
                                    product.id + '">' + product.sub_test_name +
                                    '</a>'
                            ...
                            $('#dropdownUsers').html(searchHtml);
                        } else {

                            $('#dropdownUsers').append($('<option>', {
                                    value: '',
        ...
        $(document).on('click', '.list-group-item', function() {
            ...
            $.ajax({
                ...
                success: function(response, textStatus, xhr) {
                    if (xhr.status === 200) {
                        window.location.href = APP_URL + response.redirectTo + '?data=' +
        ...
        $('.address-btn').on('click', function(e) {
            ...
            if (email === '') {
                $('.error_email').html(
                '<i class=\"icofont-info-circle\"></i> &nbsp;Email is required.');
                valid = false;
            }
            if (email) {
                if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(email)) {
                    ...
                    $('.error_email').html(
                        '<i class=\"icofont-info-circle\"></i> &nbsp;Valid Email is required.');
                    valid = false;
                }
            }
            if (phone === '') {
                $('.error_phone').html(
                '<i class=\"icofont-info-circle\"></i> &nbsp;Phone is required.');
                valid = false;
            }
            if (phone) {
                if (!$.isNumeric(phone)) {
                    $('.error_phone').html(
                        '<i class=\"icofont-info-circle\"></i> &nbsp;Phone should be numeric.');
                    valid = false;
            ...
            if (address1.trim().length === 0) {
                $('.error_address1').html(
                    '<i class=\"icofont-info-circle\"></i> &nbsp;Address is required.');
                valid = false;
            } else {
                $('.error_address1').html('');
            }
            ...
            if (state === '') {
                $('.error_state').html(
                '<i class=\"icofont-info-circle\"></i> &nbsp;State is required.');
                valid = false;
            ...
            if (zip) {
                if (!$.isNumeric(zip)) {
                    $('.error_zip').html(
                        '<i class=\"icofont-info-circle\"></i> &nbsp;Zip code should be numeric and must be 6 digits.'
                        );
                    valid = false;
                } else if (zip.trim().length > 6) {
                    $('.error_zip').html(
                        '<i class=\"icofont-info-circle\"></i> &nbsp;Zip code should be 6 digits.');
                    valid = false;
                } else {
                    $('.error_zip').html('');
                }
